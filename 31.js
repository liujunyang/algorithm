 ...
 *
 * 最优解中不包含 i 物品的话
 *  f(things数组, c) 等于f(things数组不包含 i 物品, c), 是背包体积 c 下的最大的值
 *  f(i, c) == f(i-1, c)
 *
*  最优解中包含 i 物品的话
 *  用倒推法的时候，从包里面拿出来一个 i 物品后,
 *  f(things数组, c) - value[i] 等于 f(things数组不包含 i 物品, c - w[i]), 是背包体积 c - w[i] 下最大的价值
 ...
 * f(i, c) - value[i] = f(i-1, c-w[i])
 *
 * 背包中有 i 的情况下，拿走他，价值就是体积减少相应体积后的当时的最大值？【这里要知道，对于给定的一堆物品，什么都定下了】
 *
 * c-w[i] 的体积下，确定就是少了一个 i 物品吗？【注意不是这个思路，少了一个体积，是不是 i 物品。而是已知少了个 i 物品，去得到状态转移方程】
 ...
 *
 * 倒推法比较适合人类的思维模式。
 * ！！！注意：要预防上面的"增量陷阱"：从开始向终点去思考，既不要从起点向终点思考，也不要从半截腰向终点思考。
 * ！！！不要用上面的角度从增量去考虑，需要从终点的容积 c 值全塞满去倒着考虑
 * 注意 c 可能是绰绰有余包含所有的物品的一个值，也可能是刚好包含所有物品的一个值，也可能是包含不了所有物品的一个值
 *
 *
 * 倒推时，不是说包里有两个体积相同但是价值不同的物体往外取的时候，往外拿哪个能够使得
 * 包里面剩下的总价格最大，而是说，这个 i 就是物品数组的 i，讨论 i 物品时，比如数组就是少了这个元素，
 * 就是物品堆里没有这个物体了，
 * 假如背包中就没有 i 这个东西，那么假如物品堆里没有这个物品，背包中的价值总量是不会发生变化的。：
 * f(i, c) = f(i-1, c)
 * 那假如原来 i 在包里的话，那么取出这个动作就会带来价值量和体积量的减少。
 * f(i, c) - value[i] = f(i-1, c-w[i])
 * 物品堆里没有物体 i 的话， f(数组不包含i物品了, c-w[i]) 是该条件下的最大的值。
 * f('数组没有 i 物品', c-w[i])
 *
 * 这说的情况，如果 包里有物品 i。就记住肯定有它。没有它的情况已经在上面是另外一种情况，
 * 有它的情况下，包里原来的就是个最有解，增加了它的体积后，把他放进去，是最优解。因为这就是有它的情况下进行分析的。
 *
 * 目前包里约定的肯定有i物品
 * 把 i 物品拿走之后，背包A减少 i 的体积后剩下的体积B中，如果当前的组合不是价值最大的，那么
 * 而是有别的方案，那么背包A中最沉的方案肯定不是原来的方案（虽然都包含 i 物品）。
 * 可以想象家里那个塑料袋的例子，把擦脸水拿出来后，剩下的肯定是剩下体积最重的，别的小的依然放不进去，
 * 而且也如上所说，不能换方案，否则最重的就不是原来的方案了。
 *
 * 这么说，i的遍历顺序可以正序、倒序、甚至随机跳着来，for循环只是方便遍历而已
 * 解法里面就是让i从0开始计数的，而是包的体积逐步减小去对比。
 * 就是对每个物体都试试数组里面有没有他怎么样。
 *
 ...
 *
 *
 * 我用 f(i, c) 来表示【前 i 件】物品恰好装入容量为 c 的背包中所能获得的最大价值。现在假设我试图取出的物品是 i，那么只有两种可能：

 第 i 件物品在背包里
 第 i 件物品不在背包里

 * 这句话应该改成
 * 我用 【f(数组包含i物品, c)】 来表示【这 n 件】物品恰好装入容量为 c 的背包中所能获得的最大价值。现在假设我试图取出的物品是 i，那么只有两种可能：
 ...
 *
 * 说二维数组的时候，用i-1 表示已经把 i 物品拿出来的情况的那一行。
 * 用我的理解，难以往状态转移方程上去转。
 * 2个变量，v是依赖 i 变化的，所以放内层循环。
 *
 */
funMap.knapsack = () => {
    let n = 3
