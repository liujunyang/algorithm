 ...
 * 1.在容量终点往后退
 * 2.取的是1个，所以是逐个物品讨论，讨论某个物品的时候，其他物品都在这堆物品里。
 *
 *
 ...
 *  如果包里有物品 i，即最优解中 包含 i 物品的话，就记住终点体积 c 下的最优解肯定有它。
 *  没有它的情况已经在上面是另外一种情况。
 *
 *  把 i 物品拿走之后，背包 A 减少 i 的体积后剩下的体积 B 中，
 *  如果当前的最优解剩下的物品的组合不是价值最大的，而是有别能让价值更大的方案，
 *  那么背包 A 中最沉的方案肯定不是原来的方案，
 *  就是别的方案了（虽然这个别的方案和原来的方案一样都包含 i 物品）。【记住现在一直在讨论的是情况 2】
 *  可以想象家里那个塑料袋的例子，把擦脸水拿出来后，再减少擦脸水的体积，
 *  剩下的肯定是剩下体积最重的，一个是别的物品不管大小，依然放不进去，
 *  而且也如上所说，不能换方案，否则最重的就不是原来的方案了。
 *
 * 这么说，i的遍历顺序可以正序、倒序、甚至随机跳着来，for循环只是方便遍历而已
 * 解法里面就是让 i 从 0 开始计数的，而是包的体积逐步减小去对比。
 ...
 * 不这样理解的话，难以往状态转移方程上去转。
 *
 * !!!注意是这样转的：求对于每个物品不同背包体积下的最优解，才能进行最优解之间的衔接。
 *
