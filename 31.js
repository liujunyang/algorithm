...
funMap.knapsack = () => {
    ...
    function knapsack(n, vTotal, vList, valueList) {
 ...
 * 4.在讨论 f(i, c) 的时候，就假定物品数组中的 i+1 及后面的物品都还不在可选物品堆里了。
 * 5.讨论 f(i, c) 时，就是假设取出的物品是 i。
 * 6.如果讨论物品 i-1 的话，讨论的是 f(i-1) 和 f(i-2) 的关系。而且物品 i 也不在可选物品堆里。
 ...
 *  和
 *  f(可选物品堆不包含 i 物品, c - w[i]), 是可选物品堆再刨除物品 i（即从 1 到 i -1）在背包体积 c - w[i] 下的最大的价值（该情况下的最优解）
 *  两者相等。即：
 *  f(i, c) - value[i] == f(i-1, c - w[i])
 *  进而：f(i, c) == f(i-1, c-w[i]) + value[i]
 *
 ...
 *
 *
 *  注意：这里和之前不一样的地方在于，从 i+1 到 n 的物品都不在可选物品堆了。
 *  不用全部物品的理解思路：当前一步只跟前一步有关系。过程不重要，不同的遍历顺序过程可以不一样，
 *  不过最终的最优解一样。因为最终的可选物品都是全部物品，中间用 Math.max 的状态转移方程
 *  就是在尝试各种方案。可以试试不同的遍历顺序，去查看打印过程和最终最优解。
...
funMap.knapsack3 = () => {
    ...
    function knapsack3(n, vTotal, vList, valueList) {
        // 这里数组的长度设置的 vTotal+1，因为下面初始化的 v 的值为 vTotal，
        // dp[vTotal] 想要为 0 而不是 undefined 的话，需要 dp 的 length 为 vTotal + 1
        const dp = (new Array(vTotal+1)).fill(0)
        // res 用来记录所有组合方案中的最大值
    ...
    console.log(knapsack3(n, vTotal, vList, valueList))
}
